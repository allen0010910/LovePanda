...
public class Blog extends Model<Blog> {
	...
	public Page<Blog> paginateMyBlog(int pageNumber, int pageSize, int user_id) {
		return paginate(pageNumber, pageSize, "select * ",
				"from blog where user_id=? and deleted=0 order by create_time desc", user_id);
	}

	public Page<Blog> paginateBycategory(int pageNumber, int pageSize, int category) {
		return paginate(pageNumber, pageSize, "select * ",
				"from blog where category=? and ispublic=1 and deleted=0 order by create_time desc", category);
	}

	public Page<Blog> paginateBykeyword(int pageNumber, int pageSize, String keyword) {
		return paginate(pageNumber, pageSize, "select * ", "from blog where (title like " + keyword
				+ " or keyword like " + keyword + ") and ispublic=1 and deleted=0 order by create_time desc");
	}

	public List<Blog> findUserBlogNum(int user_id) {
		return Blog.me.find("select * from blog where user_id=? and deleted=0", user_id);
	}

	// admin包下的
	public Page<Blog> adminPaginatListBlogs(int pageNumber, int pageSize, String sql) {
		return paginate(pageNumber, pageSize, "select * ",
				"from blog b where b.deleted=0 " + sql + " order by  b.create_time desc");
	}

	public Page<Blog> adminPaginatListdeletedBlogs(int pageNumber, int pageSize, String sql) {
		return paginate(pageNumber, pageSize, "select * ",
				"from blog b where b.deleted=1 " + sql + " order by  b.create_time desc");
	}
