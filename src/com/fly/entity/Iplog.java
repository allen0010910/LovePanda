...
public class Iplog extends Model<Iplog> {
	...
	public Page<Iplog> paginate(int pageNumber, int pageSize, String sql) {
		return paginate(pageNumber, pageSize, "select * ", "from iplog where 1=1 " + sql + " order by create_time asc");
	}

	public List<Record> listIps(String sql) {
		return Db.find("SELECT p.* FROM(SELECT ip,COUNT(ip)AS sumip FROM iplog WHERE 1=1 " + sql
				+ " GROUP BY ip)p ORDER BY p.sumip DESC");
	}
