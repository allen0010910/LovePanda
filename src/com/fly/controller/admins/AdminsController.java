...
public class AdminsController extends BaseController {
	...
	public void blogList() {
		...
		pagelist = Blog.me.adminPaginatListBlogs(getParaToInt("page", 1), 10, sql);
		...
		setAttr("blogPage", new Page(results, pagelist.getPageNumber(), pagelist.getPageSize(), pagelist.getTotalPage(),
	...
	public void blogDel() {
		...
		message = "删除成功";
		setAttr("message", message);
		redirectionUrl = getRequest().getContextPath() + "/admins/blogDeletedList/";
		setAttr("redirectionUrl", redirectionUrl);
		renderJsp("/WEB-INF/content/common/result.jsp");
		return;
	}
	...
	public void blogDeletedList() {
		...
		setAttr("blog", searchBlog);
		pagelist = Blog.me.adminPaginatListdeletedBlogs(getParaToInt("page", 1), 10, sql);
		List<Blog> blogs = pagelist.getList();
		List<Map> results = new ArrayList<Map>();
		for (Blog b : blogs) {
			Map every = new HashMap();
	...
	public void adviceList() {
		...
		setAttr("advicePage", new Page(result, pagelist.getPageNumber(), pagelist.getPageSize(),
				pagelist.getTotalPage(), pagelist.getTotalRow()));
	...
	public void iplogList() {
		...
		if ((empty(start) || " ".equals(start)) && (empty(end) || " ".equals(end))) {
		...
		Page<Iplog> pagelist = Iplog.me.paginate(getParaToInt("page", 1), getParaToInt("pageSize", 100), sql);
	...
	public void resourcelogList() {
		...
		Page<Resourceslog> list = Resourceslog.me.paginate(getParaToInt("page", 1), 100, sql);
	...
	public void resourcelogTongji() {
		...
		String sql = " and create_time>='" + start + "' and create_time<='" + end + "'";
		...
		for (Resourceslog rl : list) {
			...
			ynamePhyusedmemory.add((rl.getLong("phymemory") - rl.getLong("phyfreememory")) / 1024.00);
			ynameDiskmemory.add(rl.getLong("diskmemory") / 1024.00);
			ynameDiskusedmemory.add((rl.getLong("diskmemory") - rl.getLong("diskfreememory")) / 1024.00);
			ynameJVMtotal.add(rl.getLong("jvmtotalmemory") / 1024.00);
			ynameJVMused.add((rl.getLong("jvmtotalmemory") - rl.getLong("jvmfreememory")) / 1024.00);
			ynameJVMsum.add(rl.getLong("jvmmaxmemory") / 1024.00);
			ynameGC.add(rl.getLong("gccount"));
		}
		// CPU统计
		Option option = new Option();
		option.title(sdf.format(now) + " " + hour + "点段CPU使用率(X轴单位：分:秒。Y轴单位：%)");
		...
		for (int i = 0; i < xlist.size(); i++) {
			...
			xlist.get(i).axisLabel().setTextStyle(new TextStyle().fontWeight(FontWeight.bold));
		...
		optionNeicun.title(sdf.format(now) + " " + hour + "点段内存使用情况(X轴单位：分:秒。Y轴单位：GB)");
		...
		optionNeicun.toolbox().show(true).feature(Tool.dataView, Tool.restore, Tool.saveAsImage);
		...
		for (int i = 0; i < xlist.size(); i++) {
			...
			xlistPhy.get(i).axisLabel().setTextStyle(new TextStyle().fontWeight(FontWeight.bold));
		...
		optionDisk.title(sdf.format(now) + " " + hour + "点段硬盘使用情况(X轴单位：分:秒。Y轴单位：GB)");
		...
		optionDisk.toolbox().show(true).feature(Tool.dataView, Tool.restore, Tool.saveAsImage);
		...
		for (int i = 0; i < xlist.size(); i++) {
			...
			xlistDisk.get(i).axisLabel().setTextStyle(new TextStyle().fontWeight(FontWeight.bold));
		...
		optionJVM.title(sdf.format(now) + " " + hour + "点段JVM使用情况(X轴单位：分:秒。Y轴单位：GB)");
		...
		optionJVM.toolbox().show(true).feature(Tool.dataView, Tool.restore, Tool.saveAsImage);
		...
		for (int i = 0; i < xlist.size(); i++) {
			...
			xlistJVM.get(i).axisLabel().setTextStyle(new TextStyle().fontWeight(FontWeight.bold));
		...
		optionGC.title(sdf.format(now) + " " + hour + "点段GC次数(X轴单位：分:秒。Y轴单位：次)");
		...
		optionGC.toolbox().show(true).feature(Tool.dataView, Tool.restore, Tool.saveAsImage);
		...
		for (int i = 0; i < xlist.size(); i++) {
			...
			xlistGC.get(i).axisLabel().setTextStyle(new TextStyle().fontWeight(FontWeight.bold));
	...
	public void picRecommendSave() {
		...
		if (Utility.empty(uf)) {
			...
			uf.getFile().renameTo(new File(PathKit.getWebRootPath() + "/attached/picrecommend/" + filename));
		...
		setAttr("message", message);
		renderJsp("/WEB-INF/content/common/result.jsp");
		redirectionUrl = getRequest().getContextPath() + "/admins/picRecommend/";
		setAttr("redirectionUrl", redirectionUrl);
		return;
	}
	...
	public void beautyList() {
		...
		pagelist = Beauty.me.adminPaginatListBeautys(getParaToInt("page", 1), 10, sql);
		...
		for (Beauty b : blogs) {
			...
			if (!empty(b.getStr("content"))) {
				content_show = HtmlUtil.getNoHTMLString(HtmlUtil.Decode(b.getStr("content")), 150);
			}
			every.put("content", content_show);
			results.add(every);
		}
		setAttr("beautyPage", new Page(results, pagelist.getPageNumber(), pagelist.getPageSize(),
				pagelist.getTotalPage(), pagelist.getTotalRow()));
	...
	public void userList() {
		...
		pagelist = User.me.adminPaginatListUsers(getParaToInt("page", 1), 10, sql);
		...
		for (User u : users) {
			...
			if (empty(ur)) {
				...
				every.put("role", Role.me.findById(ur.getInt("role_id")).getStr("role_name"));
			}
		...
		setAttr("userPage", new Page(results, pagelist.getPageNumber(), pagelist.getPageSize(), pagelist.getTotalPage(),
				pagelist.getTotalRow()));
		render("userList.jsp");
	}
	...
	public void listDeletedUsers() {
		...
		Page<User> pagelist = null;
		pagelist = User.me.adminListDeletedUses(getParaToInt("page", 1), 10, sql);
		List<User> users = pagelist.getList();
		List<Map> results = new ArrayList<Map>();
		for (User u : users) {
			Map every = new HashMap();
	...
	public void userRecover() {
		...
		message = "解封用户成功";
		setAttr("message", message);
		redirectionUrl = getRequest().getContextPath() + "/admins/listDeletedUsers/";
		setAttr("redirectionUrl", redirectionUrl);
		renderJsp("/WEB-INF/content/common/result.jsp");
	}
	...
	public void videoList() {
		...
		pagelist = Video.me.adminPaginatListVideos(getParaToInt("page", 1), 10, sql);
		...
		setAttr("videoPage", new Page(results, pagelist.getPageNumber(), pagelist.getPageSize(),
				pagelist.getTotalPage(), pagelist.getTotalRow()));
