...
public class BlogController extends BaseController {
	...
	public void myblog() {
		...
		Page<Blog> pagelist = Blog.me.paginateMyBlog(getParaToInt(0, 1), 10, u.getInt("id"));
		List<Blog> blogs = pagelist.getList();
		List<Map> results = new ArrayList<Map>();
		for (Blog b : blogs) {
			Map every = new HashMap();
	...
	public void add() {
		...
		if (empty(getSession().getAttribute(Constants.BLOGEDITORTYPE))
				|| (Integer.valueOf(getSession().getAttribute(Constants.BLOGEDITORTYPE).toString()) == 0)) {
			render("add.jsp");
			return;
		} else if (Integer.valueOf(getSession().getAttribute(Constants.BLOGEDITORTYPE).toString()) == 1) {
	...
	@Before({ LoginInterceptor.class, BlogValidator.class, EditOpenInterceptor.class })
	public void save() {
		...
		try {
			if (!empty(file)) {
				...
				if (!uploadContentType.equals("image/png") && !uploadContentType.equals("image/jpeg")
					...
					file.getFile().renameTo(new File(PathKit.getWebRootPath() + "/attached/blog/" + filename));
	...
	public void detail() {
		...
		if (empty(u)) {
			...
			if (b.getInt("ispublic") == 0 && u.getInt("id") != b.getInt("user_id")) {
				message = "您进行了非法操作！";
				setAttr("message", message);
				renderJsp("/WEB-INF/content/common/result.jsp");
				return;
		...
		if (empty(avatar)) {
			...
			File f = new File(PathKit.getWebRootPath() + "/attached/avatar/" + avatar);
			if (!f.exists()) {
				avatar = "default.jpg";
			}
		}
	...
	public void categorySearcher() {
		...
		Page<Blog> pagelist = Blog.me.paginateBycategory(getParaToInt("page", 1), 10, category);
		List<Blog> blogs = pagelist.getList();
		List<Map> results = new ArrayList<Map>();
		for (Blog b : blogs) {
			Map every = new HashMap();
