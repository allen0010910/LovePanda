...
public class BeautyController extends BaseController {
	public void index() {
		...
		setAttr("beautyPage", new Page(results, beautylist.getPageNumber(), beautylist.getPageSize(),
				beautylist.getTotalPage(), beautylist.getTotalRow()));
		render("beauty.jsp");
	}
	...
	public void save() {
		...
		beauty.set("user_id", Constants.getLoginUser(getSession()).getInt("id"));
	...
	public void saveBeauty() {
		...
		File des = new File(PathKit.getWebRootPath() + "/attached/beauty/" + fileNmae);
		...
		try {
			...
			ImageMagicUtil.resize(saveFilename, path + "s_" + fileNmae, 180, 150);
		} catch (Exception e) {
			e.printStackTrace();
		}
		renderJson(fileNmae);
	...
	public void myBeauty() {
		...
		beautylist = Beauty.me.myBeautyPaginate(getParaToInt("page", 1), 10, user.getInt("id"));
		setAttr("ranktype", "default");
		List<Beauty> beautys = beautylist.getList();
		List<Map> results = new ArrayList<Map>();
		for (Beauty b : beautys) {
	...
	public void delete() {
		...
		message = "删除成功";
		setAttr("message", message);
		redirectionUrl = getSession().getServletContext().getContextPath() + "/beauty/myBeauty/";
