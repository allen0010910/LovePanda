...
public class VideoController extends BaseController {
	public void index() {
		...
		for (Video v : beautys) {
			...
			if (!empty(v.getStr("content"))) {
				content_show = HtmlUtil.getNoHTMLString(HtmlUtil.Decode(v.getStr("content")), 150);
			}
			every.put("content", content_show);
			results.add(every);
		}
		setAttr("videoPage", new Page(results, videolist.getPageNumber(), videolist.getPageSize(),
				videolist.getTotalPage(), videolist.getTotalRow()));
		render("video.jsp");
	}
	...
	public void save() {
		...
		File des = new File(PathKit.getWebRootPath() + "/attached/video/" + fileNmae);
		...
		try {
			...
			ImageMagicUtil.resize(saveFilename, path + "s_" + fileNmae, 200, 112);
		} catch (Exception e) {
			message = "图片裁剪异常";
			setAttr("message", message);
			renderJsp("/WEB-INF/content/common/result.jsp");
		...
		message = "保存成功";
		setAttr("message", message);
		redirectionUrl = getSession().getServletContext().getContextPath() + "/video/";
	...
	public void myVideo() {
		...
		videolist = Video.me.paginateMyVideo(getParaToInt("page", 1), 20, u.getInt("id"));
		setAttr("ranktype", "default");
		List<Video> beautys = videolist.getList();
		List<Map> results = new ArrayList<Map>();
		for (Video v : beautys) {
	...
	public void delete() {
		...
		message = "删除成功！";
		setAttr("message", message);
		redirectionUrl = getSession().getServletContext().getContextPath() + "/video/myVideo/";
		setAttr("redirectionUrl", redirectionUrl);
		renderJsp("/WEB-INF/content/common/result.jsp");
	}
